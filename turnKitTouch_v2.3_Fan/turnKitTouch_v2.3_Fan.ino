...
//  VERSION 2.3
//  Date last edited:  April 4, 2019  added variable fan timings
//
//
//
...
#define igniter 22      // igniter output on digital pin 20
#define fan 24          // fan output on digital pin 21
#define auger 26        // auger output on digital pin 22
...
int versionNumber = 2;        // no decimal places. use build number for that
int buildNumber = 3;          // build number
...
int smokeSetting[7] = {60, 65, 70, 75, 80, 85, 100};   // fan duty cycle percentage.  60 = 60% on 40% off over set duty cylce period
...
int ignitionTiming = 1600;    // sets auger OFF time for IGNITION SEQUENCE only
...
void checkTemperatures(){
  ...
  if(showTemps == true)
  {
    double grillTemp = TemperatureSensor[0]->readThermocouple(CELSIUS);
    double meatTemp = TemperatureSensor[1]->readThermocouple(CELSIUS);
